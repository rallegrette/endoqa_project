...
## Why this impresses medical-device teams
- Follows **requirements → design → implementation → verification**
- Deterministic algorithms, unit tests, and clear thresholds
- JSON reporting fits CI and lab workflows
- Easy to extend with additional QA checks (color balance, artifacts)

---

### Complexity
Let `N` = pixels per frame and `F` = frames:
- Per-frame metrics: `O(N)`
- Stuck-pixel scan: `O(N * F)`
- Trend: `O(N * F)`
Overall `O(N * F)` and memory `O(N)` for streaming analysis.

---

**Note**: This repo includes a tiny JSON builder and test framework headers to keep the project self-contained. You can swap to nlohmann/json & doctest/GoogleTest easily if preferred.
